...
.controller("ctrl.itsekbranch", ['$stateParams', '$state', '$http', '$rootScope', '$sce', 'HttpErrorService','$AspCookie','Notification',
                                 function ($stateParams, $state, $http, $rootScope, $sce, HttpErrorService, $AspCookie, Notification) {
                                     ...
                                     self.setDefault = function (brancode, isdefault) {

                                         for (var x = 0; x <= self.lstWarehouse.userBranch.length; x++) {
                                             if (self.lstWarehouse.userBranch[x].brancode === brancode && self.lstWarehouse.userBranch[x].isdefault === isdefault) {
                                                 self.lstWarehouse.userBranch[x].isdefault = true;
                                             } else {
                                                 self.lstWarehouse.userBranch[x].isdefault = false;
                                             }
                                         }
                                     }
                                     ...
                                     self.onViewUserWarehouse = function (p) {
                                         $http.get(coreapi + 'user/getUserWarehouse/?userId=' + p + '&sysId=22').
                                       success(function (data, status, headers, config) {
